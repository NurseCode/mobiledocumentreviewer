...
- **PDF Manipulation Tools Tab**: Complete suite of PDF manipulation tools:
  ...
  - **Sign Document**: Three signature methods - finger draw (full-screen pad via top-level overlay), typed cursive font (4 styles), and photo upload. Full-screen draw pad rendered at MainScreen level via `LocalOpenSigningPad`/`LocalSignatureBitmapResult`/`LocalConsumeSignatureResult` CompositionLocals to survive screen rotation. Child composables set `awaitingSignatureFromPad` flag and react via `LaunchedEffect` when the bitmap result arrives. Includes drag-to-position placement with resize slider and multi-page navigation.
